   ...
   <nav class="navbar navbar-expand-lg bg-white navbar-light shadow sticky-top p-0">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!address) {
                console.log('Adresse non fournie dans l\'URL, utilisation d\'une adresse par défaut...');
                address = 'Avenue Habib Bourguiba, Tunis, Tunisie'; // Adresse par défaut pour les tests
                window.history.pushState({}, document.title, `?address=${encodeURIComponent(address)}`);
            }
            ...
            function displayMap(address) {
                ...
                const defaultLocation = [36.8065, 10.1815]; // Tunis
                const map = L.map('map').setView(defaultLocation, 6); // Zoom initial sur la Tunisie
                L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                    attribution: '© OpenStreetMap contributors'
                }).addTo(map);

                // Limiter la recherche à la Tunisie avec countrycodes=TN
                fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address)}&countrycodes=TN`)
                    .then(response => response.json())
                    .then(data => {
                        if (data && data.length > 0) {
                            const location = [data[0].lat, data[0].lon];
                            map.setView(location, 15); // Zoom sur l'emplacement trouvé
                            const marker = L.marker(location).addTo(map);
                            marker.bindPopup(decodeURIComponent(address)).openPopup();
                        } else {
                            document.getElementById('error-message').textContent = 'Adresse non trouvée en Tunisie';
