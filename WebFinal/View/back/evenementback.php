...
<div class="mb-3">
    ...
    <input type="number" class="form-control" id="nbplace" name="nbplace" min="1" step="1">
    <span class="error text-danger" id="errorNbPlace"></span>
</div>
    ...
    <tbody id="eventTableBody">
        <?php foreach ($evenements as $evenement): ?>
            <tr>
                ...
                <td><?= htmlspecialchars($evenement['nbplace']) ?></td>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="evenementback.php" method="POST" novalidate onsubmit="return validateForm(this)">
                                ...
                                <div class="mb-3">
                                    <label for="nbplace" class="form-label">Nombre de places</label>
                                    <input type="number" class="form-control" id="nbplace" name="nbplace" value="<?= htmlspecialchars($evenement['nbplace'] ?? '') ?>" min="1" step="1" required>
                                    <div id="nbplaceError" class="text-danger"></div>
                                 </div>
...
<script>
    ...
    document.getElementById('nbplaceError').textContent = '';
    ...
    // Vérification de la catégorie (lettres, y compris accents, et espaces)
    const categorie = form['categorie'].value;
    if (!/^[a-zA-ZÀ-ÿ\s]+$/.test(categorie)) {
    ...
    // Vérification du lieu (lettres, chiffres, espaces et virgules)
    const lieu = form['lieu'].value;
    if (!/^[a-zA-Z0-9\s,]+$/.test(lieu)) {
        document.getElementById('lieuError').textContent = 'Le lieu ne doit contenir que des lettres, des chiffres, des espaces et des virgules.';
        isValid = false;
    }
    ...
    // Vérification du nombre de places (non vide, entier strictement supérieur à 1)
    const nbplace = form['nbplace'].value;
    if (nbplace === '') {
        isValid = false; // Champ vide : empêcher l'envoi sans afficher de message
    } else if (!/^\d+$/.test(nbplace) || parseInt(nbplace) <= 1) {
        document.getElementById('nbplaceError').textContent = 'Le nombre de places doit être un entier strictement supérieur à 1.';
        isValid = false;
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        const nbplace = document.getElementById('nbplace').value.trim();
        ...
        const regexLieu = /^[a-zA-Z0-9\s,]+$/; // Lettres, chiffres, espaces et virgules pour lieu
        ...
        // Nombre de places
        if (nbplace === "") {
            document.getElementById('errorNbPlace').textContent = "Le nombre de places est obligatoire.";
            valid = false;
        } else if (!/^[0-9]+$/.test(nbplace) || parseInt(nbplace) < 1) {
            document.getElementById('errorNbPlace').textContent = "Le nombre de places doit être supérieur à 1.";
            valid = false;
        }
